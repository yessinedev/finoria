...
import type { ClientPayment, Invoice, Client } from "@/types/types";
...
export default function ClientPayments() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = db.subscribe((table, action) => {
      if (["client_payments", "invoices"].includes(table)) {
        loadData();
      }
    });
  ...
  const loadData = async () => {
    ...
    try {
      const [paymentsResult, invoicesResult, clientsResult] = await Promise.all([
        db.payments.clientPayments.getAll(),
        db.invoices.getAll(),
        db.clients.getAll(),
      ]);
  ...
  const handleDeletePayment = async (payment: ClientPayment) => {
    ...
    try {
      const result = await db.payments.clientPayments.delete(payment.id);
      if (result.success) {
        toast({
          title: "Succès",
  ...
  return (
    <div className="p-6 flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Paiements clients</h1>
          <p className="text-muted-foreground">
            Gérez tous les paiements reçus de vos clients
          </p>
        </div>
