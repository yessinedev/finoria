...
module.exports = (ipcMain, db, notifyDataChange) => {
  ...
  ipcMain.handle("create-credit-note", async (event, creditNote) => {
    try {
      ...
      // If the credit note is created with "Confirmée" status, track stock movements immediately
      if (creditNote.status === "Confirmée") {
      ...
      throw new Error(
        "Erreur lors de la création de la facture d'avoir: " + error.message
      );
    }
  });
  ...
  ipcMain.handle("update-credit-note-status", async (event, id, status) => {
    try {
      ...
      const currentCreditNote = db
        .prepare("SELECT * FROM credit_notes WHERE id = ?")
        .get(id);

      // Update the credit note status
      const stmt = db.prepare(
        "UPDATE credit_notes SET status = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?"
      );
      ...
      if (result.changes === 0) {
        throw new Error(
          "Facture d'avoir non trouvée ou statut déjà mis à jour"
        );
      }
      ...
      if (status === "Confirmée" && currentCreditNote.status !== "Confirmée") {
        ...
        const items = db
          .prepare("SELECT * FROM credit_note_items WHERE creditNoteId = ?")
          .all(id);
      ...
      // If the status is being changed from "Confirmée" to another status, remove the stock
      else if (
        currentCreditNote.status === "Confirmée" &&
        status !== "Confirmée"
      ) {
        // Get credit note items
        const items = db
        ...
        // Remove stock for each item (subtract the quantity that was credited)
        const updateStockStmt = db.prepare(`
          UPDATE products 
          SET stock = stock - ? 
          WHERE id = ? AND category != 'Service'
        ...
        // Create stock movement records for the removal
        const insertMovementStmt = db.prepare(`
          INSERT INTO stock_movements (
            productId, productName, quantity, movementType, sourceType, sourceId, reference, reason
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ...
        for (const item of items) {
          // Only remove stock for non-service products
          updateStockStmt.run(item.quantity, item.productId);
          ...
          insertMovementStmt.run(
            ...
            `CREDIT-${id}`,
            status === "Annulée"
              ? "Annulation de facture d'avoir"
              : "Modification de statut de facture d'avoir"
          );
        }
      }
      ...
      throw new Error(
        "Erreur lors de la mise à jour du statut de la facture d'avoir: " +
          error.message
      );
    }
  });
  ...
  ipcMain.handle(
    ...
    async (event, invoiceId, reason) => {
      ...
      `);
        ...
        // Track stock movements since the credit note is created with "Confirmée" status
        // Return stock for each item (add back the quantity that was credited)
        const updateStockStmt = db.prepare(`
        UPDATE products 
        SET stock = stock + ? 
      ...
      `);
        ...
        throw new Error(
          "Erreur lors de la génération de la facture d'avoir à partir de la facture"
        );
      }
    }
  );
  ...
  ipcMain.handle("get-credit-note-items", async (event, creditNoteId) => {
    try {
      ...
      throw new Error(
        "Erreur lors de la récupération des articles de la facture d'avoir"
      );
    }
  });
};
